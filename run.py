...
def main(args):
    ...
    logger = utils.setup_logger(__name__, os.path.join(args.checkpoint_dir, 'train.20190726.maskQuesNo.t9.log'))
    ...
    if not args.infer:
        ...
        for j in range(args.epochs):
            ...
            with (checkpoint_path / 'model{:03d}.t9'.format(j)).open('wb') as f:
                torch.save(model, f)
            ...
            if val_pk < best_val_pk:
                ...
                logger.debug(
                    colored(
                        ...
                        'green'))
                best_val_pk = val_pk
                with (checkpoint_path / 'best_model.t9'.format(j)).open('wb') as f:
                    torch.save(model, f)
