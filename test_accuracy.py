    ...
    for dataset_path in dataset_folders:
        ...
        with tqdm(desc='Testing', total=len(dl)) as pbar:
            ...
            for i, (data, targets, paths) in enumerate(dl):
                ...
                pbar.update()
                path, loc_list, ocr_list = paths[0]
                path, pid = str(path), str(path).split("/")[-1].split(".")[0]
                output = model(data)
                ...
                if args.bs == 1:
                    if args.seg_folder:
                        output_path = "./output/%s.%d"%(args.seg_folder, args.mark)
                    else:
                        output_path = "./output/%s.%d"%('test', args.mark)
                    output_path = output_path[:-1] if output_path[-1]=="/" else output_path
                    cmd = "mkdir -p %s"%output_path
                    os.system(cmd)
                    if True or len(target_seg) > batch_accurate:
                        with open("%s/%s.label"%(output_path, pid), 'w') as fp:
                            for i, flag in enumerate(target_seg):
                                fp.write("%s\n"%("\t".join([loc_list[i], ocr_list[i]]).encode("UTF-8")))
                                if target_seg[i]:
                                    fp.write("===######===\n")
                        with open("%s/%s.pred"%(output_path, pid), 'w') as fp:
                            for i, flag in enumerate(output_seg):
                                fp.write("%s\n"%("\t".join([loc_list[i], ocr_list[i]]).encode("UTF-8")))
                                if output_seg[i]:
                                    fp.write("===######===\n")
                        # print i, paths, targets
                        # print i, "output_seg", output_seg
                        # print i, "target_seg", target_seg
                        print "batch_accurate=%.2f, sum=%d, len=%d"%(batch_accurate*1.0/len(target_seg), batch_accurate, len(target_seg))
