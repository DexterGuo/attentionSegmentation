...
class Accuracies(object):
    def __init__(self):
        self.thresholds = np.arange(0.2, 0.7, 0.02)
...
def main(args):
    ...
    logger = utils.setup_logger(__name__, os.path.join(args.checkpoint_dir, 'train.%d.log'%args.mark))
    ...
    if not args.infer:
        dataset_path = Path(utils.config['segdataset'])
        train_dataset = SegTextDataSet(dataset_path / 'train', word2vec=word2vec,
                high_granularity=args.high_granularity, max_token_num=args.max_token_num)
        dev_dataset = SegTextDataSet(dataset_path / 'dev', word2vec=word2vec, 
                high_granularity=args.high_granularity, max_token_num=args.max_token_num)
        test_dataset = SegTextDataSet(dataset_path / 'test', word2vec=word2vec,
                high_granularity=args.high_granularity, max_token_num=args.max_token_num)
    ...
    if not args.infer:
        ...
        for j in range(args.epochs):
            ...
            with (checkpoint_path / 'model{:03d}.t{}'.format(j, args.mark)).open('wb') as f:
                torch.save(model, f)
            ...
            if val_pk < best_val_pk:
                ...
                logger.debug(
                    colored(
                        ...
                        'green'))
                best_val_pk = val_pk
                with (checkpoint_path / 'best_model.t{}'.format(args.mark)).open('wb') as f:
                    torch.save(model, f)

    else:
        test_dataset = SegTextDataSet(args.infer, word2vec=word2vec,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seg', help='Use seg as dataset?', action='store_true')
