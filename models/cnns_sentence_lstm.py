...
class SentenceEncodingCNN(nn.Module):
    def __init__(self, D = 300, Ci = 1, Co = 32, Ks = [2,3]):
        ...
        self.n_layers = 5
        self.conv = nn.Conv2d(1, 2*D, (3, D), padding=(1,0) )
    ...
    def forward(self, x):
        ...
        for _ in range(self.n_layers):
            x_ = x 
            x = F.dropout(x, p=0.1)
            x = self.conv(x).transpose(1,3)  #B, 1, W, 2*D) 
            x = F.glu(x, 3)
            x = (x + x_)*math.sqrt(0.5)
...
class Model(nn.Module):
    def __init__(self, sentence_encoder, shidden=32*2, hidden=128, num_layers=2, Ks=[2, 3]):
    ...
    def forward(self, batch):
        ...
        if max_length < self.Ks[-1] : max_length = self.Ks[-1]
...
def create():
    ...
    return Model(sentence_encoder, shidden=len(Ks) * Co, hidden = 256, num_layers=2, Ks=Ks)
