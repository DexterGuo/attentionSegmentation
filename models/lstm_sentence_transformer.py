...
from transformer.Models import SentTrans 
...
class Model(nn.Module):
    def __init__(self, sentence_encoder, transformer_model, d_model):
    ...
    def forward(self, batch):
        ...
        #print ("---------->", type(encoded_sentences), encoded_sentences.size())
        ...
        docs_tensor = torch.cat(padded_docs, 1).permute(1, 0, 2)
        #print ("--------->", type(docs_tensor), docs_tensor.size())
        #packed_docs = pack_padded_sequence(docs_tensor, ordered_doc_sizes)
        unsorted_doc_outputs = self.transformer_model(docs_tensor, max_doc_size, doc_sizes)
        unsorted_doc_outputs = unsorted_doc_outputs.permute(1, 0, 2)
        #sentence_lstm_output, _ = self.transformer_model(packed_docs, zero_state(self, batch_size=batch_size))
        ...
        for i, doc_len in enumerate(doc_sizes):
            doc_outputs.append(unsorted_doc_outputs[0:doc_len - 1, i, :])  # -1 to remove last prediction

        #unsorted_doc_outputs = [doc_outputs[i] for i in unsort(ordered_document_idx)]
        #print ("------->", type(unsorted_doc_outputs), unsorted_doc_outputs.size())
...
def create():
    sentence_encoder = SentenceEncodingRNN(input_size=300,
                                           hidden=256,
                                           num_layers=2)
    transformer_model = SentTrans(len_max_seq=10000, d_word_vec=512, n_layers=2, n_head=8, 
                                  d_k=64, d_v=64, d_model=512, d_inner=2048, dropout=0.1)
    return Model(sentence_encoder, transformer_model, d_model=512)
