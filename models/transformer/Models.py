...
def get_non_pad_mask(seq, lengths):
    batch, len_q = seq.size(0), seq.size(1)
    assert len(lengths) == batch
    return maybe_cuda(torch.tensor([[1 if i<clen else 0 for i in range(len_q) ] for clen in lengths]).type(torch.float).unsqueeze(-1) )
...
class SentTrans(nn.Module):
    ...
    def __init__(
        ...
        super(SentTrans, self).__init__()

        n_position = len_max_seq + 1

        self.position_enc = nn.Embedding.from_pretrained(
            #get_sinusoid_encoding_table(n_position, d_word_vec, padding_idx=0),
            get_sinusoid_encoding_table(n_position, d_word_vec, padding_idx=None),
            freeze=True)
